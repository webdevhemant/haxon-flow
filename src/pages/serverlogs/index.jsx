...
const MOCK_LOGS = Array.from({ length: 60 }, (_, i) => {
    ...
    const messages = {
        info: [
            ...
            'Cache hit for node result',
            'Webhook delivered successfully',
            'Model response received in 420ms'
        ],
        debug: [
            'Node input: {"query":"..."}',
        ...
        warn: [
            ...
            'Context length nearing max',
            'Response latency high (>2s)',
            'Deprecated node version detected'
        ],
    ...
    return {
        ...
        source: ['flowise-server', 'flow-executor', 'api-handler', 'cache-layer'][Math.floor(Math.random() * 4)]
    }
}).reverse()
                ...
                <div ref={logRef} className='h-[520px] overflow-y-auto p-4 space-y-1 font-mono text-xs'>
                    {filtered.map((log) => (
                        <div
                            key={log.id}
                            className={cn(
                                'flex items-start gap-3 px-2 py-1 rounded hover:bg-white/5 transition-colors group',
