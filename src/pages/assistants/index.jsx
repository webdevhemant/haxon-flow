...
const MODEL_COLORS = { 'gpt-4o': '#10B981', 'gpt-4o-mini': '#22D3EE', 'claude-opus-4-5': '#6366F1', 'claude-sonnet-4-5': '#A855F7', 'gemini-2.0-flash': '#F59E0B', 'llama-3.3-70b': '#EF4444' }
const AVATARS = ['🤖', '🧠', '💡', '⚡', '🎯', '🔮', '🦾', '🌟']
...
function AssistantDialog({ open, onClose, onSave, initial }) {
    ...
    const handleSave = () => {
        ...
        setName(''); setDesc(''); setModel('gpt-4o'); setAvatar('🤖'); setSysPrompt('You are a helpful AI assistant.')
    }
...
export default function Assistants() {
    ...
    const filtered = items.filter((a) => a.name.toLowerCase().includes(search.toLowerCase()) || a.description?.toLowerCase().includes(search.toLowerCase()))

    const handleAdd = (data) => { setItems((p) => [{ ...data, id: `ast-${Date.now()}` }, ...p]); setShowAdd(false); toast.success('Assistant created') }
    const handleEdit = (data) => { setItems((p) => p.map((a) => a.id === editing.id ? { ...a, ...data } : a)); setEditing(null); toast.success('Updated') }
    const handleDelete = (id) => { setItems((p) => p.filter((a) => a.id !== id)); toast.success('Deleted') }
    const handleDuplicate = (a) => { setItems((p) => [{ ...a, id: `ast-${Date.now()}`, name: a.name + ' (Copy)', conversations: 0 }, ...p]); toast.success('Duplicated') }
        ...
        <div className='space-y-6 animate-fade-in'>
            ...
            <div className='flex flex-col sm:flex-row sm:items-center gap-4'>
                <div className='relative flex-1 max-w-xs'>
                    ...
                    <Input placeholder='Search assistants...' className='pl-8 h-8 text-sm' value={search} onChange={(e) => setSearch(e.target.value)} />
                </div>
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                {filtered.map((a, i) => {
                    ...
                    return (
                        <Card key={a.id} className={cn('card-hover overflow-hidden group animate-slide-up', `stagger-${Math.min(i + 1, 8)}`)}>
                            <div className='h-0.5' style={{ background: color }} />
                            <CardContent className='p-5'>
                                <div className='flex items-start justify-between mb-4'>
                                    <div className='h-12 w-12 rounded-xl flex items-center justify-center text-2xl' style={{ background: color + '18', border: `1px solid ${color}28` }}>
                                        {a.avatar || '🤖'}
                                    </div>
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <Button variant='ghost' size='icon-sm' className='opacity-0 group-hover:opacity-100 transition-opacity'><IconDots size={14} /></Button>
                                        </DropdownMenuTrigger>
                                        <DropdownMenuContent align='end'>
                                            <DropdownMenuItem onClick={() => setEditing(a)}><IconEdit size={13} /> Edit</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => handleDuplicate(a)}><IconCopy size={13} /> Duplicate</DropdownMenuItem>
                                            <DropdownMenuItem className='text-destructive focus:text-destructive' onClick={() => handleDelete(a.id)}><IconTrash size={13} /> Delete</DropdownMenuItem>
                                        </DropdownMenuContent>
                                    </DropdownMenu>
                                </div>
                                ...
                                <div className='flex items-center gap-2 mb-4'>
                                    <Badge style={{ background: color + '18', color }} className='border-0 text-[10px] font-mono'>{a.model}</Badge>
            ...
            {filtered.length === 0 && (
                <div className='flex flex-col items-center justify-center py-20 text-center'>
                    ...
                    <Button variant='gradient' onClick={() => setShowAdd(true)}><IconPlus size={14} /> New Assistant</Button>
                </div>
            )}
        </div>
    )
