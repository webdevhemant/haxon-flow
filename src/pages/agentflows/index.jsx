...
import { IconPlus, IconSearch, IconDots, IconRobot, IconPlayerPlay, IconEdit, IconCopy, IconTrash, IconBolt, IconCpu, IconBrain, IconUsersGroup } from '@tabler/icons-react'
                ...
                <div className='space-y-4 py-2'>
                    ...
                    <div className='space-y-1.5'>
                        ...
                        <select value={model} onChange={(e) => setModel(e.target.value)}
                            ...
                            {['gpt-4o', 'gpt-4o-mini', 'claude-3-5-sonnet', 'claude-3-opus', 'gemini-1.5-pro'].map((m) => <option key={m} value={m}>{m}</option>)}
                        </select>
                    </div>
                </div>
                <DialogFooter>
...
export default function Agentflows() {
    ...
    const handleDelete = (id) => { deleteAgentflow(id); toast.success('Agent deleted') }
    const handleDuplicate = (flow) => { addAgentflow({ ...flow, name: flow.name + ' (Copy)', deployed: false }); toast.success('Duplicated') }
        ...
        <div className='space-y-6 animate-fade-in'>
            ...
            <div className='grid grid-cols-2 sm:grid-cols-4 gap-3'>
                {[
                    ...
                    { label: 'Total Runs', value: agentflows.reduce((a, f) => a + (f.executionCount || 0), 0).toLocaleString(), icon: IconPlayerPlay, color: 'text-cyan' },
                    { label: 'Avg Nodes', value: agentflows.length ? Math.round(agentflows.reduce((a, f) => a + (f.nodeCount || 0), 0) / agentflows.length) : 0, icon: IconBrain, color: 'text-accent' },
                ].map((s) => (
                    ...
                    return (
                        <div key={flow.id} className={cn('card-hover glass rounded-xl border border-border overflow-hidden group animate-slide-up', `stagger-${Math.min(i + 1, 8)}`)}>
                            ...
                            <div className='p-4'>
                                <div className='flex items-start justify-between mb-3'>
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align='end'>
                                            ...
                                            <DropdownMenuItem onClick={() => { updateAgentflow(flow.id, { deployed: !flow.deployed }); toast.success(flow.deployed ? 'Deactivated' : 'Activated!') }}>
                                ...
                                <Button variant='outline' size='sm' className='w-full text-xs h-7 mt-1' onClick={() => navigate(`/canvas/${flow.id}`)}>
