                    ...
                    <Card key={store.id} className={cn('card-hover overflow-hidden group animate-slide-up', `stagger-${Math.min(i + 1, 8)}`)}>
                        ...
                        <CardContent className='p-5'>
                                ...
                                <div className='flex items-center gap-2'>
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align='end'>
                                            <DropdownMenuItem onClick={() => toast.info('Coming soon!')}><IconEdit size={13} /> Manage</DropdownMenuItem>
                                            <DropdownMenuItem onClick={() => toast.info('Re-indexing...')}><IconRefresh size={13} /> Re-index</DropdownMenuItem>
                                            <DropdownMenuItem className='text-destructive focus:text-destructive' onClick={() => { setStores((p) => p.filter((s) => s.id !== store.id)); toast.success('Deleted') }}>
                            ...
                            <div className='flex items-center justify-between text-xs text-muted-foreground pt-3 border-t border-border'>
                                <span className='flex items-center gap-1'><IconFileText size={11} /><span className='font-mono'>{store.docCount.toLocaleString()}</span> docs</span>
                                <span className='flex items-center gap-1'><IconBolt size={11} className='text-primary' /><span className='font-mono'>{store.chunkCount.toLocaleString()}</span> chunks</span>
                            </div>
                        </CardContent>
                    </Card>
                ))}
