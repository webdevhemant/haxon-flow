...
const CATEGORY_COLORS = {
    Support: '#6366F1', Research: '#22D3EE', DevTools: '#10B981', Data: '#A855F7',
    Marketing: '#F59E0B', HR: '#EF4444', Legal: '#06B6D4', Productivity: '#8B5CF6', Finance: '#F59E0B',
}
        ...
        <div className='space-y-8 animate-fade-in'>
            ...
            <div className='rounded-xl border border-primary/20 bg-primary/5 p-5 relative overflow-hidden'>
                ...
                <div className='relative flex flex-col sm:flex-row sm:items-center gap-4'>
                    <div className='flex-1'>
                        ...
                        <p className='text-sm text-muted-foreground'>Production-ready flows built by the Haxon team and top community contributors.</p>
                    </div>
                ...
                <div className='flex gap-2'>
                    {['stars', 'downloads'].map((s) => (
                        <button
                            key={s}
                            onClick={() => setSort(s)}
                            className={cn('rounded-lg px-3 py-1.5 text-xs transition-colors capitalize', sort === s ? 'bg-secondary text-foreground' : 'text-muted-foreground hover:text-foreground')}
                        >
            ...
            <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                {filtered.map((m, i) => {
                    ...
                    return (
                        <Card key={m.id} className={cn('card-hover group overflow-hidden animate-slide-up', `stagger-${Math.min(i + 1, 8)}`)}>
                            <div className='h-0.5' style={{ background: catColor }} />
                            <CardContent className='p-5'>
                                <div className='flex items-start justify-between mb-3'>
                                    <Badge style={{ background: catColor + '18', color: catColor }} className='border-0 text-[10px]'>{m.category}</Badge>
                                    {m.featured && <Badge variant='cyan' className='text-[10px]'>Featured</Badge>}
                                </div>
                                ...
                                <div className='flex items-center justify-between pt-3 border-t border-border'>
                                    <div className='flex items-center gap-3 text-xs text-muted-foreground'>
                                        <span className='flex items-center gap-1'><IconStar size={11} className='text-warning' /><span className='font-mono'>{m.stars}</span></span>
                                        <span className='flex items-center gap-1'><IconDownload size={11} /><span className='font-mono'>{m.downloads.toLocaleString()}</span></span>
                                    </div>
                                    <Button variant='outline' size='sm' className='h-7 text-xs' onClick={() => toast.success(`Installing "${m.name}"...`)}>
