        ...
        <div className='space-y-6 animate-fade-in'>
            ...
            <div className='flex flex-col sm:flex-row gap-3'>
                <div className='relative flex-1 max-w-xs'>
                    ...
                    <Input placeholder='Search by flow name...' className='pl-8 h-8 text-sm' value={search} onChange={(e) => setSearch(e.target.value)} />
                </div>
                    ...
                    {['all', 'success', 'error', 'running'].map((s) => (
                        <button key={s} onClick={() => setStatusFilter(s)}
                            className={cn('rounded-full px-3 py-1 text-xs font-medium transition-all capitalize',
                                statusFilter === s ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20' : 'bg-secondary text-muted-foreground border border-border hover:text-foreground'
                            )}>
                            {s}
                        </button>
                    ))}
                ...
                <table className='w-full text-sm'>
                    <thead>
                        <tr className='border-b border-border bg-secondary/40'>
                            ...
                            <th className='text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground'>Flow</th>
                            <th className='text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground hidden md:table-cell'>Duration</th>
                            <th className='text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground hidden md:table-cell'>Tokens</th>
                            <th className='text-left px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground hidden lg:table-cell'>Time</th>
                        </tr>
                    </thead>
                    <tbody>
                        {filtered.slice(0, 30).map((ex, i) => {
                            ...
                            const Icon = cfg.icon
                            return (
                                <tr key={ex.id} className='border-b border-border hover:bg-secondary/30 transition-colors animate-slide-up' style={{ animationDelay: `${i * 0.02}s` }}>
                                    <td className='px-4 py-3'>
                                        <div className={cn('flex h-6 w-6 items-center justify-center rounded-full',
                                            ex.status === 'success' ? 'bg-success/15 text-success' :
                                            ex.status === 'error' ? 'bg-destructive/15 text-destructive' : 'bg-warning/15 text-warning'
                                        )}>
                                            <Icon size={10} strokeWidth={3} />
                                        </div>
                                    </td>
                                    <td className='px-4 py-3 font-medium text-foreground'>{ex.flow}</td>
                                    <td className='px-4 py-3 font-mono text-xs text-muted-foreground hidden md:table-cell'>{ex.duration}ms</td>
                                    <td className='px-4 py-3 font-mono text-xs text-muted-foreground hidden md:table-cell'>{ex.tokens.toLocaleString()}</td>
                                    <td className='px-4 py-3 text-xs text-muted-foreground hidden lg:table-cell'>{formatRelativeTime(ex.time)}</td>
                                </tr>
                            )
                        })}
                    </tbody>
