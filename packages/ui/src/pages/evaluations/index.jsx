        ...
        <div className='space-y-6 animate-fade-in'>
            ...
            <div className='space-y-3'>
                {filtered.map((ev, i) => {
                    ...
                    return (
                        <div key={ev.id} className={cn('glass rounded-xl border border-border p-4 hover:border-primary/20 transition-colors animate-slide-up', `stagger-${Math.min(i + 1, 8)}`)}>
                            <div className='flex items-start justify-between mb-3'>
                                <div className='flex items-center gap-3'>
                                    <div className={cn('flex h-7 w-7 items-center justify-center rounded-full',
                                        ev.status === 'passed' ? 'bg-success/15 text-success' :
                                        ev.status === 'failed' ? 'bg-destructive/15 text-destructive' : 'bg-warning/15 text-warning'
                                    )}>
                            ...
                            {ev.score !== null && (
                                <div className='mt-3 h-1 bg-secondary rounded-full overflow-hidden'>
                                    <div className={cn('h-full rounded-full transition-all', ev.score >= 0.9 ? 'bg-success' : ev.score >= 0.7 ? 'bg-warning' : 'bg-destructive')}
