...
const MODEL_COLORS = {
    'gpt-4o': '#10B981', 'gpt-4o-mini': '#22D3EE', 'claude-3-5-sonnet': '#A855F7',
    'claude-opus-4': '#6366F1', 'gemini-pro': '#F59E0B',
}
        ...
        <div className='space-y-6 animate-fade-in'>
                        ...
                        <Card key={a.id} className={cn('card-hover overflow-hidden group animate-slide-up', `stagger-${Math.min(i + 1, 8)}`)}>
                            ...
                            <CardContent className='p-5'>
                                <div className='flex items-start justify-between mb-4'>
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align='end'>
                                            ...
                                            <DropdownMenuItem onClick={() => toast.success('Duplicated!')}><IconCopy size={13} /> Duplicate</DropdownMenuItem>
                                            <DropdownMenuItem className='text-destructive focus:text-destructive' onClick={() => { setItems((p) => p.filter((x) => x.id !== a.id)); toast.success('Deleted') }}>
                                ...
                                <div className='flex items-center justify-between text-xs text-muted-foreground pt-3 border-t border-border'>
                                    ...
                                    <Button variant='outline' size='sm' className='h-6 text-[10px]' onClick={() => toast.info('Chat interface coming soon!')}>
                                        Try it
                                    </Button>
                                </div>
                            </CardContent>
            ...
            {filtered.length === 0 && (
                <div className='flex flex-col items-center justify-center py-20 text-center'>
                    ...
                    <Button variant='gradient' onClick={() => toast.info('Coming soon!')}><IconPlus size={14} /> New Assistant</Button>
                </div>
            )}
        </div>
    )
