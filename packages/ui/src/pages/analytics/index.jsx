...
const STAT_CARDS = [
    { label: 'Total Executions', key: 'totalExecutions', delta: 'executionsDelta', icon: IconActivity, color: '#6366F1', format: (v) => v.toLocaleString() },
    { label: 'Tokens Used', key: 'totalTokens', delta: 'tokensDelta', icon: IconDatabase, color: '#22D3EE', format: (v) => (v / 1_000_000).toFixed(1) + 'M' },
                        ...
                        <Card key={c.key} className={cn('overflow-hidden animate-slide-up', `stagger-${i + 1}`)}>
                            ...
                            <CardContent className='p-5'>
                                <div className='flex items-center justify-between mb-3'>
                                    ...
                                    <div className={cn('flex items-center gap-1 text-xs font-mono', isPositive ? 'text-success' : 'text-destructive')}>
                                        {isPositive ? <IconTrendingUp size={12} /> : <IconTrendingDown size={12} />}
                                        {Math.abs(delta)}{typeof delta === 'number' && delta < 100 ? '%' : ''}
                                    </div>
                                </div>
                    ...
                    <CardContent className='pt-4'>
                        <ResponsiveContainer width='100%' height={220}>
                            <AreaChart data={executionTimeline.slice(-20)} margin={{ top: 5, right: 5, bottom: 0, left: -20 }}>
                                ...
                                <Area type='monotone' dataKey='executions' stroke='#6366F1' fill='url(#execGrad)' strokeWidth={2} dot={false} />
                ...
                <Card className='animate-slide-up stagger-4'>
                    ...
                    <CardContent className='pt-4 flex flex-col items-center'>
                        <ResponsiveContainer width='100%' height={140}>
                            <PieChart>
                                <Pie data={flowTypeDistribution} cx='50%' cy='50%' innerRadius={40} outerRadius={60} dataKey='value' strokeWidth={0}>
                    ...
                    <CardContent className='pt-4'>
                        <ResponsiveContainer width='100%' height={160}>
                            <BarChart data={tokensByModel} layout='vertical' margin={{ top: 0, right: 0, bottom: 0, left: 0 }}>
                                ...
                                <YAxis type='category' dataKey='model' tick={{ fontSize: 11, fill: '#64748B' }} tickLine={false} axisLine={false} width={80} />
                                <Tooltip content={<CustomTooltip />} />
                        ...
                        <div className='divide-y divide-border'>
                            {recentExecutions.map((ex) => (
                                <div key={ex.id} className='flex items-center gap-3 px-5 py-3 hover:bg-secondary/30 transition-colors'>
                                    <div className={cn('flex h-5 w-5 shrink-0 items-center justify-center rounded-full text-xs',
                                        ...
                                        {ex.status === 'success' ? <IconCheck size={10} strokeWidth={3} /> : <IconX size={10} strokeWidth={3} />}
                                    </div>
                                    <div className='flex-1 min-w-0'>
                                        <div className='text-sm text-foreground truncate'>{ex.flow}</div>
                                        <div className='text-xs text-muted-foreground font-mono'>{ex.duration}ms · {ex.tokens.toLocaleString()} tokens</div>
                                    </div>
